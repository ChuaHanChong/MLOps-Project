...
Current implemented augmentation technique:
    1) AutoAugment
       Reference: https://arxiv.org/abs/1805.09501
    2) RandAugment
       Reference: https://arxiv.org/abs/1909.13719
https://github.com/google/automl/blob/master/efficientdet/aug/autoaugment.py
"""
import inspect
import math
...
def rotate(image, degrees, replace):
    """
    Rotate the image by degrees either clockwise or counterclockwise.
...
def autocontrast(image):
    """
    Implement Autocontrast function from PIL using TF ops.
...
def sharpness(image, factor):
    """Implement Sharpness function from PIL using TF ops."""
    orig_image = image
...
def equalize(image):
    """Implement Equalize function from PIL using TF ops."""
...
def _shrink_level_to_arg(level):
    """Convert level to ratio by which we shrink the image content."""
    if level == 0:
...
def _parse_policy_info(name, prob, level, replace_value, augmentation_hparams):
    ...
    It return the function that corresponds to `name` and update `level` param.
    """
    func = NAME_TO_FUNC[name]
    args = level_to_arg(augmentation_hparams)[name](level)

    # Check to see if prob is passed into function.
    # This is used for operations where we alter bboxes independently.
...
def build_and_apply_nas_policy(policies, image, augmentation_hparams):
    ...
    policies : list
        ...
        `func` is a string name of the augmentation function,
        `prob` is the probability of applying the `func` operation,
        `level` is the input argument for `func`.
    image : tf.Tensor
        The tensor that the resulting policy will be applied to.
    augmentation_hparams :
        Hparams associated with the NAS learned policy.
...
def distort_image_with_autoaugment(image, augmentation_name):
    """
    Apply the AutoAugment policy to `image`.
    ...
    if augmentation_name not in available_policies:
        raise ValueError(
            f'Invalid augmentation_name: {augmentation_name}',
        )
...
def distort_image_with_randaugment(image, num_layers, magnitude):
    """
    Apply the RandAugment policy to `image`.
...
def distort_image(image, aug_name, ra_num_layers, ra_magnitude):
    ...
    if aug_name == 'autoaug':
        ...
        image = tf.cond(
            tf.equal(i, 1),
            lambda: distort_image_with_randaugment(
                image,
                ra_num_layers,
                ra_magnitude,
            ),
